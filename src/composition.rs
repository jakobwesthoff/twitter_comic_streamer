...
pub async fn create_composition_image() -> DynamicImage {
    ...
    if drawing_instructions.len() == 1 {
        ...
        let dy =
            ((COMPOSITION_HEIGHT - drawing_instructions[0].area.h) as f64 / 2.0).round() as u32;
    ...
    let mut target =
        ImageBuffer::from_pixel(COMPOSITION_WIDTH, COMPOSITION_HEIGHT, background_color);
